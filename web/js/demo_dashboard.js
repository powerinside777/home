$(function(){        
    ...
    function getData() {
        $.getJSON('http://10.0.0.57:8084/temperature_query.json?num_obs=-1&start_date=2013-01-18T16:00', function (data) {
            var series = {
                data: []
            }
            ...
            while (data[0].temperature_record[0][i]) {
                series.data.push([data[0].temperature_record[0][i].unix_time, data[0].temperature_record[0][i].celsius]);
                i++;

            }
            currenttemp = data[0].temperature_record[0][i - 1].celsius
